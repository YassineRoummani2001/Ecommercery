...
<div className={` items-center  justify-between w-full md:flex md:w-auto ${isModalView ? "" : " hidden" } `} id='nav-items'>
    ...
    {isAdminView
        ? adminNavOptions.map((item) => (
            ...
            className=" bg-transparent cursor-pointer block py-2 pr-4 pl-3 text-gray-900 rounded md:p-0 "
            >
            {item.label}
            </li>
        ))
...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="backdrop-blur-sm bg-white/30 fixed w-full z-20 top-0 left-0 border-b border-gray-50">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <div onClick={()=>router.push('/')} className="flex items-center cursor-pointer">
         <span className="self-center text-2xl text-black font-semibold whitespace-nowrap">
             <span  className="self-center text-2xl font-semibold whitespace-nowrap text-orange-400">E</span>commercery
